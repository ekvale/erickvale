...
that command writes "Sample summary for ..." placeholder text and assigns
tags at random, so filtering by a tag returns an arbitrary set and the pages
read as noise. This one carries hand-written summaries and abstracts, and
each publication is tagged with terms that actually describe it, so facet
and tag filtering demonstrates something true.
...
DEMO_PUBLICATIONS = [
    (
        ...
        "outside the Twin Cities metro, by county and age group.",
        "Screening completion outside the metro trails statewide rates across "
        "all three cancers examined, with the widest gap among adults aged 50 "
        "to 64 in counties without a local screening program. Travel distance "
        "to the nearest facility is the strongest single correlate.",
        ["cancer", "screening-programs", "greater-minnesota", "rural-communities",
         "adults", "transportation-barriers"],
    ),
    (
        "Opioid Overdose Deaths: Annual Surveillance Summary",
        "Surveillance Summary", "2025-06-30", "published", True,
        "Statewide overdose mortality, including drug type, county, and "
        "demographic breakdowns, with naloxone distribution figures.",
        "Overdose deaths involving synthetic opioids continued to account for "
        "the majority of drug deaths. Rates remained highest among American "
        "Indian residents. Counties with saturated naloxone distribution "
        "recorded lower fatality-to-encounter ratios.",
    ...
    (
        ...
        "Legislative Report", "2025-03-01", "published", True,
        "Case review of pregnancy-associated deaths, contributing factors, "
        "and prevention recommendations submitted to the legislature.",
        "The committee reviewed pregnancy-associated deaths and judged a "
        "majority preventable. Mental health conditions and hemorrhage were "
        "the leading underlying causes. Disparities by race persisted after "
        "adjusting for age and insurance status.",
    ...
    (
        ...
        "of Minnesotans who remain uninsured.",
        "The uninsured rate held roughly steady statewide, masking increases "
        "among young adults and self-employed workers. Cost remains the most "
        "commonly cited reason for going without coverage.",
    ...
    (
        ...
        "clinicians practicing outside metropolitan areas.",
        "More than a third of rural primary care physicians are within ten "
        "years of expected retirement, concentrated in counties already "
        "designated as shortage areas. Advanced practice clinicians have "
        "absorbed much of the growth in visit volume.",
    ...
    (
        ...
        "after an elevated result, by housing age and region.",
        "Testing coverage varies substantially by county. Elevated results "
        "cluster in census tracts with the oldest housing stock. Follow-up "
        "testing within the recommended window occurred in fewer than half "
        "of elevated cases.",
    ...
    (
        ...
        "role of connection to school and trusted adults.",
        "Students reporting a trusted adult at school were substantially less "
        "likely to report persistent sadness or suicidal ideation. The "
        "association held across grade, region, and family income.",
    ...
    (
        ...
        "Occupancy recovered unevenly, with rural facilities reporting the "
        "greatest difficulty filling direct care roles. Several facilities "
        "reduced licensed capacity rather than operate understaffed.",
    ...
    (
        ...
        "water systems, with guidance for operators.",
        "Detections were concentrated near known industrial and firefighting "
        "foam use. Guidance covers sampling frequency, treatment options, and "
        "public notification thresholds for affected systems.",
    ...
    (
        ...
        "availability following the end of emergency flexibilities.",
        "Telehealth use settled well above pre-2020 levels but far below its "
        "peak, with behavioral health retaining the highest share. Counties "
        "with weaker broadband saw the sharpest decline.",
    ...
    (
        ...
        "Statistical Table", "2024-04-10", "published", False,
        "School-reported immunization and exemption rates by district and "
        "vaccine series.",
        "Statewide coverage declined modestly for the second consecutive "
        "year. Conscientious exemptions rose in a small number of districts, "
        "several of which fall below the threshold for measles herd immunity.",
        ["immunization-programs", "vaccine-preventable-disease", "children",
         "education", "registries", "county-level"],
    ),
    (
        "Food Insecurity and Health Outcomes",
        "White Paper", "2024-02-28", "published", False,
        "The relationship between household food insecurity and chronic "
        "disease management, drawing on survey and clinical data.",
        "Food-insecure households reported worse diabetes and hypertension "
        "control. Participation in nutrition assistance was associated with "
        "narrowed but not eliminated differences.",
        ["food-security", "diabetes", "cardiovascular-disease",
         "income-poverty", "health-disparities", "survey-data"],
    ),
    (
        ...
        "covering governance, data sharing, and program reach.",
        "Data sharing agreements that recognized tribal data sovereignty were "
        "associated with sustained participation. Programs designed with "
        "tribal health directors from the outset reached more households.",
    ...
    (
        ...
        "Surveillance Summary", "2023-11-02", "published", False,
        "Fatal and non-fatal firearm injuries by intent, age, and county.",
        "Suicide accounted for the majority of firearm deaths, concentrated "
        "among older men in rural counties. Non-fatal assault injuries were "
        "concentrated among young men in urban counties.",
        ["firearm-injury", "suicide-prevention", "older-adults",
         "community-safety", "disease-surveillance"],
    ),
    (
        "Prescription Drug Price Transparency",
        "Annual Report", "2023-09-25", "published", False,
        "Reported price increases and new high-cost drug launches under the "
        "state transparency statute.",
        "Manufacturers reported price increases exceeding the statutory "
        "threshold on a substantial number of products. Specialty drugs "
        "accounted for a disproportionate share of total reported spending.",
    ...
    (
        ...
        "Visit rates rose sharply above a heat index threshold, with the "
        "steepest increases among adults over 65 and outdoor workers. "
        "Neighborhoods with less tree canopy recorded higher rates.",
    ...
    (
        ...
        "Centers served a growing share of uninsured and publicly insured "
        "patients while maintaining quality measure performance at or above "
        "statewide averages for several chronic disease indicators.",
    ...
    (
        ...
        "Annual Report", "2023-04-19", "published", False,
        "Outbreaks investigated during the year, by pathogen, setting, and "
        "implicated food.",
        "Investigations spanned restaurant, institutional, and private event "
        "settings. Whole genome sequencing linked several geographically "
        "dispersed clusters that traditional interviews had not connected.",
    ...
    (
        ...
        "Toolkit", "2023-02-21", "published", False,
        "Practical guidance for health systems on interpreter services, "
        "translated materials, and measuring language access.",
        "The toolkit covers assessing patient language needs, contracting "
        "qualified interpreters, and avoiding reliance on family members. "
        "Includes sample policies and a self-assessment checklist.",
    ...
    (
        ...
        "Dashboard", "2025-11-10", "published", False,
        "Weekly influenza, COVID-19, and RSV activity with hospitalization "
        "and test positivity trends.",
        "The dashboard reports weekly activity by region and age group, "
        "drawing on sentinel provider reporting, laboratory results, and "
        "hospital admissions.",
    ...
    (
        ...
        "visiting, including birth outcomes and developmental screening.",
        "Enrolled families completed developmental screening at higher rates "
        "than comparison families. Preterm birth differences were not "
        "statistically significant at current sample sizes.",
    ...
    (
        ...
        "unmet dental need by region.",
        "A minority of practicing dentists accept new publicly insured adult "
        "patients. Unmet need is highest in counties without a community "
        "dental clinic.",
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for (
            title, type_name, iso_date, status, featured, summary, abstract, tag_slugs
